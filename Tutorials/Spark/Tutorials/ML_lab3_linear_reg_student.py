...
def parsePoint(line):
    ...
    Returns:
        LabeledPoint: The line is converted into a `LabeledPoint`, which consists of a label and
            features.
    """
    return LabeledPoint(float(line.split(',', 1)[0]), map(float, line.split(',', 1)[1].split(',')))
...
def twoWayInteractions(lp):
    ...
    return LabeledPoint(lp.label, 
                        np.hstack((lp.features, map(lambda x: x[0] * x[1], 
                                                    list(itertools.product(lp.features, lp.features))))))
